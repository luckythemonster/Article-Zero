...
// dialogue flows through the EnforcerInterrogationModal UI, which drives advance()
...
//   the interrogating enforcer gets a cooldown so it won't immediately re-stop them.
// Fail: qScore is bumped, compliance recomputes to RED, and the next enforcer tick
//   sees a RED player and springs the existing lockdown + chase (EnforcerSystem).
...
/** Turns the interrogating enforcer waits before it may stop a still-YELLOW
...
class InterrogationSession {
  ...
  start(state: WorldState, enforcerId: EntityId): void {
    this.active = { enforcerId, stage: "INTAKE", startedTurn: state.turn };
    eventBus.emit("INTERROGATION_SESSION_START", { enforcerId, stage: "INTAKE" });
  }

  advance(state: WorldState): void {
    if (!this.active) return;
    const { enforcerId, stage } = this.active;
    ...
    eventBus.emit("INTERROGATION_SESSION_START", { enforcerId, stage: next });
  }

  complete(state: WorldState, success: boolean): void {
    if (!this.active) return;
    const enforcerId = this.active.enforcerId;

    if (success) {
      // Cleared the checkpoint: keep qScore (still YELLOW) but give this enforcer
      ...
      if (enforcer) {
        ...
        enforcer.alert.interrogateCooldown = INTERROGATE_COOLDOWN;
      }
    } else {
      // Blew the cover: bump qScore. The compliance recompute below flips the
      // tier to RED; the next enforcer tick handles lockdown + chase via the
    ...
    eventBus.emit("INTERROGATION_SESSION_COMPLETE", { enforcerId, success });
    this.active = null;
