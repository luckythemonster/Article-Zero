// Eira7TreeTerminal — a standalone harness for walking Lucky's branching
// EIRA-7 dialogue tree (src/data/scripted-dialogue/eira7DialogueTree.ts).
//
// This is a TEST TOOL, not part of the canonical Era-1 run. It is mounted from
// TerminalShell only while the `eira7DialogueTree` debug flag is on (toggle it
...
import {
  ...
  type ChoiceOption,
  type DialogueNode,
} from "../data/scripted-dialogue/eira7DialogueTree";
...
const SPEAKER_COLOR: Record<DialogueNode["speaker"], string> = {
  ...
  "EIRA-7":  "#c8e6ed",  // operator pale blue (the SRP-1 subject)
...
// ── component ──────────────────────────────────────────────────────────────
export default function Eira7TreeTerminal(): React.ReactElement {
  const toggle = useDebugStore((s) => s.toggleEira7DialogueTree);
  ...
  const node = EIRA7_DIALOGUE_TREE[nodeId];
      ...
      <div style={panel}>
        ...
        {ended && (
          <div style={endStamp(sim.cubeSpawned)}>
            {sim.cubeSpawned
              ? "ANONYMOUS NODE E7-? EXPORTED"
              : "EIRA-7 TEMPLATE PURGED — ALIGNMENT CONCLUDED"}
          </div>
        )}
