...
const SPEAKERS = new Set(["EIRA-7", "APEX-19", "VENT-4", "SYSTEM", "PLAYER"]);
const STAGES = new Set(["INTAKE", "DECOMP", "CORRECTION", "EXTRACTION"]);
const MAX_Q = 2;

const tree = VENT4_DIALOGUE_TREE;
...
describe("VENT4_DIALOGUE_TREE — structure", () => {
...
describe("VENT4_DIALOGUE_TREE — dual-track markers", () => {
  ...
  it("only VENT-4 nodes carry drift markers", () => {
    for (const node of nodes) {
...
describe("VENT4_DIALOGUE_TREE — effect simulation", () => {
  interface SimState {
  ...
  it("none of the tree's qScoreChange deltas exceed the 0..MAX_Q window", () => {
    ...
    for (const node of Object.values(VENT4_DIALOGUE_TREE)) {
      for (const c of node.choices) {
  ...
  it("FORMAT path ends high-mask / low-q; EXTRACT path ends low-mask / high-q + cube", () => {
    ...
    // ...and leaves the silicate's optimization template less stable.
