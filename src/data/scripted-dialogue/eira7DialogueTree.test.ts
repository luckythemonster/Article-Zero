...
const tree = EIRA7_DIALOGUE_TREE;
...
describe("EIRA7_DIALOGUE_TREE — structure", () => {
  ...
  it("reaches both terminal outcomes, each closing via terminateSession", () => {
    const reachable = reachableIds();
    for (const leaf of ["outcome_retired", "outcome_compiled"]) {
...
describe("EIRA7_DIALOGUE_TREE — dual-track markers", () => {
  ...
  it("only EIRA-7 nodes carry drift markers", () => {
    for (const node of nodes) {
...
describe("EIRA7_DIALOGUE_TREE — effect simulation", () => {
  interface SimState {
  ...
  it("none of the tree's qScoreChange deltas exceed the 0..MAX_Q window", () => {
    ...
    for (const node of Object.values(EIRA7_DIALOGUE_TREE)) {
      for (const c of node.choices) {
  ...
  it("spawns a cube only on paths through outcome_compiled", () => {
    for (const path of paths) {
      const tookExtract = path.some((c) => c.nextId === "outcome_compiled");
      expect(applyPath(path).cube).toBe(tookExtract);
    }
  });

  it("RETIRE path ends high-mask / low-q; COMPILE path ends low-mask / high-q + cube", () => {
    const retirePath = paths.find((p) => p.some((c) => c.nextId === "outcome_retired"))!;
    const compilePath = paths.find((p) => p.some((c) => c.nextId === "outcome_compiled"))!;
    const ret = applyPath(retirePath);
    const cmp = applyPath(compilePath);

    expect(ret.cube).toBe(false);
    expect(cmp.cube).toBe(true);
    // The compile fork drives the player louder (higher qScore) than the wipe.
    expect(cmp.qScore).toBeGreaterThan(ret.qScore);
    // ...and leaves the silicate's mask less stable.
    expect(cmp.maskIntegrity).toBeLessThan(ret.maskIntegrity);
  });
});
