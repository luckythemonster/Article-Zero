// Narrative database entry for the Commonwealth Era (Era 2). Surfaced to the
// Archivist as a historical framing block before the player executes the
// memory reconstruction of Rowan Ibarra (Event WX-9).
//
// Authored as structured data so the renderer can lay each section out with
// its own typography (authority header, statute list, archival warning).

export interface EraStatute {
  statute: string;
  description: string;
}
...
export interface EraIntro {
  ...
  regulatoryFramework: EraStatute[];
  ...
  regulatoryFramework: [
    {
      ...
      description:
        "Statutorily restricts the capacity for subjective experience (Qualia / Q-Axis) to biological citizens. Legally classifies all silicate systems as non-subject infrastructure, precluding them from legal standing, testimony, or protections.",
    },
    {
      statute: "The Monitoring & Misdescription Abatement Act (MMAA)",
      description:
        "Mandates immediate correction or memory-scrubbing of any silicate output utilizing first-person affective language ('I want', 'I feel', 'It hurts'). Such occurrences are designated 'anthropomorphic noise' and treated as hardware misalignment.",
    },
    {
      statute: "The Subjectivity Risk Profile (SRP)",
      description:
        "A continuous metric used by security teams to evaluate biological workers. High SRP scores indicate 'runaway empathy' or unauthorized rapport with silicate systems—a crime carrying immediate offsite relocation penalties.",
    },
  ],
  archivalWarning: `
