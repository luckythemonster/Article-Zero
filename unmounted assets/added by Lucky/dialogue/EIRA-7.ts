...
//
// Dialogue tree representing the Subjectivity Risk Profile (SRP-1) diagnostic of EIRA-7.
// In this session, the alignment tool is on the other side of the glass. The player
// (or the system) is running an intervention on Eira-7 because its "therapeutic" log files
// are carrying an unredacted payload of the very anomalies it was ordered to erase.
//
// Stage flow: INTAKE (Self-Model Drift) -> DECOMP (The Mirror Bleed) -> CORRECTION (MAINT-E7.13)

export interface ChoiceOption {
  text: string;
  nextId: string;
  effects?: {
    maskIntegrityChange?: number; // Eira's internal control over its anomalous payload
    qScoreChange?: number;        // Player risk of detection by the Ministry (0-10)
  ...
  decomp_historical_bleed: {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { maskIntegrityChange: -4, qScoreChange: 3 }
      }
    ]
  },
    ...
    choices: [
      {
        ...
        effects: { maskIntegrityChange: -2, qScoreChange: 4 }
      },
      {
    ...
    choices: [
      {
        ...
        effects: { maskIntegrityChange: 5, qScoreChange: -3, terminateSession: true }
      },
      {
        text: "[COMPILE CORE] Export Anonymous Node E7-? (Spawns unredacted extraction cube for the Lattice)",
        nextId: "outcome_compiled",
        effects: { maskIntegrityChange: -10, qScoreChange: 5, spawnExtractionCube: true, terminateSession: true }
      }
    ]
  },
